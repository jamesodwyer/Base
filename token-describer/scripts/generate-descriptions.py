...
def generate_description_for_path(path_parts, file_context):
    ...
    if file_context.startswith("component/"):
        ...
        if path_parts[0] == "layout" and len(path_parts) >= 3:
            key = path_parts[2]
            layout_descs = {
                "inBetweenCards": "Gap between card elements.",
                "inBetweenInteractive": "Gap between interactive elements.",
...
def process_token_tree(data, path_parts, file_context, stats):
    """
    ...
    if is_token(data):
        ...
        if desc:
            ...
            if existing:
                ...
                # Update if too long (>60) or too short (<20)
                if len(existing) > 60 or len(existing) < 20:
    ...
    for key in data:
        ...
        if isinstance(child, dict):
            process_token_tree(child, path_parts + [key], file_context, stats)
...
def process_file(file_path, stats):
    ...
    process_token_tree(data, [], file_context, file_stats)
    ...
    print(f"  Updated (was too long): {file_stats['updated']}")
    ...
    for key in ("added", "updated", "kept_existing", "no_match"):
...
def main():
    ...
    print(f"Descriptions updated:  {stats['updated']}")
    print(f"Kept existing:         {stats['kept_existing']}")
    print(f"No pattern match:      {stats['no_match']}")
    total = stats["added"] + stats["updated"] + stats["kept_existing"] + stats["no_match"]
    covered = stats["added"] + stats["updated"] + stats["kept_existing"]
