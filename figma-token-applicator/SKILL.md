...
- **Pill Button** — 72 variants. Node: `21:12205`. Stripped 2026-05-07. New token files: `tokens/component/pillButton.json` (Light + Inverse colour states) and `pillButton.spacing.{medium,small}.{blockPadding,inlinePadding,gap}` appended to `component/spacing/desktop.json`. Selected state uses new `selected.accent.fill.{default,hover}` semantic token (turquoise + darken modifier). Per-variant binding: `.BASE / Pill` carries padding (4 sides) + gap + borderRadius (`borderRadius.interactive.full`) + fill + (Default-only and Inverse-Disabled-only) borderColor/borderWidth (`border.interactive.xSmall`); Text carries `typography.label.medium` + fill; Chevron INSTANCE fill bound, inner Icon vectors untouched. State map: Default→default, Hover→hover, Pressed→pressed, Disabled→disabled, Selected→selected.default, Selected Hover→selected.hover. JIRA: GDS-445.
